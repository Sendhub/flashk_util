"""
The authdigest module contains classes to support digest authentication compliant with RFC 2617.
...
This implementation is based on the "RealmDigestDB" implementation by the Werkzeug Team/Shane Holloway.

    See: https://github.com/shanewholloway/werkzeug/blob/master/werkzeug/contrib/authdigest.py
...
class RealmDigestDb:
    ...
    Args:
        realm: String identifying the hashing realm.
        algorithm: String identifying hash algorithm to use, default is 'md5'.
    """
    ...
    def to_dict(self):
        """To dict"""
        _r = {"cfg": {"algorithm": self.alg.algorithm, "realm": self.realm}, "db": self._db}
        return _r
    ...
    def is_authenticated(self, request, **kw):
        ...
        if not self.alg.verify(authorization, hash_pass, request.method, **kw):
...
class AuthenticationResult:
    ...
    def __repr__(self):
        return "<authenticated: %r reason: %r>" % (self.authenticated, self.reason)
...
class DigestAuthentication:
    ...
    def _compute_ha1(self, auth, password=None):
        return self.hash_password(auth.username, auth.realm, password or auth.password)
