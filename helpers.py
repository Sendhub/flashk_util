...
def jsonify(*args, **kwargs):
    """
    Create a :class:`~flask.Response` with the JSON representation of the given arguments with an `application/json` mimetype. The arguments to this function are the same as to the :class:`dict` constructor.
    ...
    This requires Python 2.6 or an installed version of simplejson. For security reasons only objects are supported toplevel. For more information about this, have a look at :ref:`json-security`.
    ...
    response = current_app.response_class(
        json.dumps(dict(*args, **kwargs), indent=None if request.headers.get("X-Requested-With") == "XMLHttpRequest" else 4, default=defaultEncoder), mimetype="application/json"
    )
...
def get_view_window_params(*params):
    ...
    out = []
    for param in params:
        assert param in ("offset", "limit", "sort", "order"), f'Requested parameter "{param}" is not available'
        value = None
        if param == "offset":
...
def csvify(*_args, **kwargs):
    ...
    return current_app.response_class(generate_csv(), mimetype="text/csv", headers=response_headers)
